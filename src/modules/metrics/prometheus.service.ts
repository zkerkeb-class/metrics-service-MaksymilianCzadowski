...
export class PrometheusService implements OnModuleInit {
  ...
  // HTTP Metrics
  incrementHttpRequests(method: string, route: string, statusCode: string, service: string) {
    this.httpRequestsTotal.inc({ method, route, status_code: statusCode, service });
  }

  observeHttpDuration(method: string, route: string, service: string, duration: number) {
    this.httpRequestDuration.observe({ method, route, service }, duration);
  }

  // Service Health Metrics
  setServiceHealth(serviceName: string, serviceUrl: string, isHealthy: boolean) {
  ...
  setServiceResponseTime(serviceName: string, responseTime: number) {
    this.serviceResponseTime.set({ service_name: serviceName }, responseTime / 1000); // Convert to seconds
  }
